...
export default async ({ strapi }: { strapi: Strapi }) => {
  ...
  if(!res) {
    await strapi.query('plugin::strapi-react-email.react-email-template').create({
      data: { originCode: '', transpiledCode: '', isDefault: true, name: 'Reset password', locale: locale.code, slug: slugify('Reset password') }
    });
    await strapi.query('plugin::strapi-react-email.react-email-template').create({
      data: { originCode: '', transpiledCode: '', isDefault: true, name: 'Email address confirmation', locale: locale.code, slug: slugify('Email address confirmation') }
    });
  }
};
